    ...
    <div className="flex flex-col gap-10 h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 pb-24">
        {filteredCombos.map((combo) => (
          <Card key={combo.id} onClick={() => setSelectedCombo(combo)} className="group overflow-hidden rounded-[3rem] border-none shadow-sm hover:shadow-2xl transition-all duration-500 bg-white flex flex-col h-full cursor-pointer border-t-4 border-t-primary/30">
            ...
            <div className="p-8 flex flex-col flex-1">
              <div className="flex justify-between items-start mb-2"><h3 className="text-xl font-black">{combo.name}</h3><span className="text-xl font-black text-primary">${combo.basePrice.toFixed(2)}</span></div>
              <p className="text-sm text-muted-foreground mb-6 line-clamp-2">{combo.description || "Choice set combo."}</p>
              <div className="mt-auto flex justify-between items-center"><Badge variant="outline" className="text-[10px] font-black uppercase">Combo</Badge><Plus className="h-5 w-5 text-primary opacity-0 group-hover:opacity-100" /></div>
            </div>
          </Card>
        ))}
        ...
        {filteredPackages.map((pkg) => (
          <Card key={pkg.id} onClick={() => addPackageToCart(pkg)} className="group overflow-hidden rounded-[3rem] border-none shadow-sm hover:shadow-2xl transition-all duration-500 bg-white flex flex-col h-full cursor-pointer border-t-4 border-t-accent/30">
            ...
            <div className="p-8 flex flex-col flex-1">
              <div className="flex justify-between items-start mb-2"><h3 className="text-xl font-black">{pkg.name}</h3><span className="text-xl font-black text-primary">${pkg.price.toFixed(2)}</span></div>
              <p className="text-sm text-muted-foreground mb-6 line-clamp-2">{pkg.description || "Product bundle."}</p>
              <div className="mt-auto flex justify-between items-center"><Badge variant="secondary" className="bg-accent/10 text-accent font-black">Bundle</Badge><Plus className="h-5 w-5 text-primary opacity-0 group-hover:opacity-100" /></div>
            </div>
          </Card>
        ))}
              ...
              <div className="p-8 flex flex-col flex-1">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-right">
                    ...
                    {activePriceList && <p className="text-[9px] text-accent font-bold">Tier Pricing</p>}
                  </div>
                </div>
                ...
                <div className="mt-auto flex justify-between items-center">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className={cn("text-[10px] font-bold mt-1", stockLeft < 10 ? "text-orange-500" : "text-muted-foreground")}>Stock: {stockLeft}</span>
                  </div>
      ...
      <Dialog open={!!selectedCombo} onOpenChange={(open) => !open && setSelectedCombo(null)}>
        <DialogContent className="max-w-2xl rounded-[2.5rem] p-10">
                      ...
                      {group.options.map((opt) => {
                         ...
                         return (
                            <button key={opt.productId} onClick={() => setComboSelections(prev => ({ ...prev, [group.id]: opt.productId }))} className={cn("flex items-center justify-between p-4 rounded-2xl border-2 transition-all", isSelected ? "bg-primary/5 border-primary shadow-lg" : "bg-white border-muted/50 hover:border-primary/20")}>
                               <div className="flex items-center gap-3">{isSelected ? <CheckCircle2 className="text-primary h-5 w-5" /> : <Circle className="text-muted/30 h-5 w-5" />}<div className="text-left"><p className="font-bold text-sm">{p?.name || 'Item'}</p>{opt.extraPrice > 0 && <p className="text-[10px] font-black text-primary">+$ {opt.extraPrice.toFixed(2)}</p>}</div></div>
                            </button>
                         );
                      })}
                   </div>
          ...
          <DialogFooter className="mt-10"><Button onClick={handleAddComboToCart} disabled={!selectedCombo?.groups.every(g => !g.required || !!comboSelections[g.id])} className="w-full h-16 rounded-2xl bg-primary font-black text-lg">Add Combo to Order</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
