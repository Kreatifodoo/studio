...
import { Plus, Package, Tag, Layers, Box } from 'lucide-react';
...
export function ProductGrid() {
  const { 
    ...
    products, categories, cart, priceLists, packages 
  } = usePOS();
  ...
  return (
    <div className="flex flex-col gap-10 h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-10 pb-24">
        {/* Render Packages First if applicable */}
                ...
                <div className="mb-6 p-4 bg-muted/20 rounded-2xl">
                   ...
                   <div className="flex flex-wrap gap-2">
                      {pkg.items.map((item, idx) => {
                        ...
                        return (
                          <Badge key={idx} variant="outline" className="bg-white border-none text-[10px] font-bold px-2 py-0.5 rounded-lg shadow-sm">
        ...
        {filteredProducts.length === 0 && filteredPackages.length === 0 && (
          <div className="col-span-full py-20 text-center">
            <p className="text-muted-foreground font-bold">No products or packages found for this search.</p>
          </div>
        )}
      </div>
    </div>
